...
def select_frames_kmeans(session: Session, num_frames_to_pick: int, indices: Optional[List[int]]=None, num_clusters: int=None, chunk_size: int=1000,
    ...
    indices (List[int]|None): Subset of indices to pick cluster from
    ...
    if indices is None:
        ...
        frames_iterator = session.index(indices, chunk_size=chunk_size)
        nframes = len(indices)

    downsampled = np.zeros((nframes, int(roi.shape[0] / scale), int(roi.shape[1] / scale)))
    all_frame_idxs = np.empty((nframes,), dtype=int)
